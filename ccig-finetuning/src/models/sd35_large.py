...
class SD35LargeTrainer(DiffusersLoraTrainer):
    ...
    frozen_module_names = ["vae", "text_encoder", "text_encoder_2", "text_encoder_3"]
    ...
    def _training_step(self, pipe: Any, transformer: Any, batch: dict) -> torch.Tensor:
        ...
        # transformer's memory budget from having to share the GPU with the ~8B+ params
        # of frozen SD3.5-large weights (three text encoders incl. a T5) at the same time.
        def _encode():
            with torch.no_grad():
            ...
            return noisy_latents, noise, latents, timesteps, prompt_embeds, pooled_prompt_embeds

        noisy_latents, noise, latents, timesteps, prompt_embeds, pooled_prompt_embeds = (
