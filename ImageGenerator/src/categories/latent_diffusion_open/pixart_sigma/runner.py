"""PixArt-Sigma runner: base inference + LoRA fine-tuning (diffusers + PEFT on transformer)."""
...
def _collate_pixart(batch: list[tuple[Image.Image, str]], resolution: int = 1024):
...
@dataclass
class PixArtSigmaRunner(Runner):
    model_id: str = "pixart_sigma"
    category: Category = "latent_diffusion_open"

    _pipe: Optional[PixArtAlphaPipeline] = None

    def _get_pipeline(self) -> PixArtAlphaPipeline:
    ...
    def _get_pipeline_with_lora(self, ckpt_dir: str) -> PixArtAlphaPipeline:
    ...
    def finetune(
        ...
        *,
        ...
        train_loader = DataLoader(
            ...
            shuffle=True,
            num_workers=0,
            collate_fn=lambda b: _collate_pixart(b, resolution=resolution),
        )
        ...
        try:
            pbar = tqdm(total=config.max_steps, desc="LoRA finetune pixart_sigma")
            ...
            raise RuntimeError(
                "GPU OOM during PixArt fine-tuning. Try: --resolution 512, "
