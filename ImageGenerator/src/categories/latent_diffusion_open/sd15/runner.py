"""SD1.5 runner: base inference + LoRA fine-tuning (diffusers + PEFT)."""
...
def _collate_sd15(batch: list[tuple[Image.Image, str]], resolution: int = 512):
...
class SD15Runner(Runner):
    ...
    def _get_pipeline_with_lora(self, ckpt_dir: str) -> StableDiffusionPipeline:
        ...
        if not adapters_path.exists():
            raise FileNotFoundError(
                f"Adapters not found at {adapters_path}. "
                "Run finetune first to produce adapters."
            )
    ...
    def finetune(
        ...
        *,
        ...
        resolution = config.resolution or 512
        try:
            ...
            raise FileNotFoundError(
                f"Dataset or images root invalid: {e}. "
                "Check --data and --images_root paths."
            ) from e
        ...
        try:
            pbar = tqdm(total=config.max_steps, desc="LoRA finetune sd15")
            for pixel_values, texts in _cycle(train_loader):
                ...
                with torch.no_grad():
                    ...
                    # Some pipeline versions return (positive_embeds, negative_embeds); UNet needs a single tensor
            ...
            raise RuntimeError(
                "GPU OOM during fine-tuning. Try: --resolution 512 (if not already), "
                "--batch_size 1, --grad_accum 8, or use a smaller model."
            ) from e
